...
- Configure one Base64-encoded 32-byte `CredentialEncryption__Key` value on each authorized host. Connection administrators enter or replace referenced database/FTP credentials in the Web UI; THub stores only AES-GCM ciphertext in SQL and never reads a stored value back to the browser.
- System Administrators manage trusted webhook and executable definitions under `/trusted-actions`. Grant `trusted-action.use` for the resulting resource ID to a custom role before its members can publish a workflow using it. Optional webhook/run-as credentials use the same encrypted store and external master key.
