...
- Editor sessions may change approved values but cannot change structure, formatting, formulas, images, charts, hyperlinks, imports, validation rules, clipboard contents, or autofill. Undo/redo remains available, and staged values are normalized and revalidated server-side.
- The editor initially loads 250 rows and offers 250, 500, 1,000, or 2,000 rows per page up to the immutable active version's maximum. Parameterized provider-specific counts make the current and total page count explicit while keyset navigation remains authoritative.
- Editable cells are driven by the active version's immutable data type, nullability, declared length, numeric precision, and numeric scale. Temporal cells offer a picker and locale-aware text parsing. Client-side feedback is an early usability check only; typed Application validation, parameterized mutation binding, and source database constraints remain authoritative.
- A Windows-authenticated user with the publication's `View` grant may export all readable columns and every source row to one XLSX worksheet. Export reads database state rather than unsaved browser edits, uses bounded keyset batches and a delete-on-close temporary file, fails above Excel's row or cell limits, and never silently truncates.
...
The management UI now supports bounded SQL table/view discovery, immutable version activation, transactional role-grant replacement, and token management. The Windows-authenticated editor implements bounded deterministic pages, explicit page totals, governed full-table XLSX export, read-only viewer behavior, metadata-driven typed insert/update/delete staging, change-set list/detail/review, and governed foreign-key lookup cells. Views and objects without a usable key remain read-only. The current editor does not provide workbook import, arbitrary workbook export operations, formulas, direct writes, or automatic replay after an ambiguous source commit.

## Email alerts, actions, and data destinations

[ADR-0027](adr/0027-email-workflow-destination.md) keeps Email behind the governed durable outbox and adds a bounded tabular data destination.
...
- Running-run terminal transitions and direct queued-run cancellation commit deduplicated workflow-rule outbox rows with the run state after rechecking the complete matching rule/profile snapshot. The `EmailAlert` action and `EmailTarget` destination queue through the same outbox and succeed when durable intent exists, not when delivery is confirmed; their run/node deduplication identity remains stable across recovered step attempts.
- `EmailTarget` accepts one tabular input. Inline mode requires exactly one `{{data}}` placeholder and HTML-encodes every column name and cell. Attachment mode writes one safely named UTF-8 CSV file. Both modes fail rather than truncate above 10,000 rows, the absolute body bound, or a 10 MiB attachment.
- The prepared inline table or attachment is durable message content in `AlertDeliveries.MessageJson`. Restrict SQL access and backups accordingly, monitor outbox capacity, and apply the retention/classification decision required by PD-009. Never copy this content to logs, normalized errors, audit metadata, or operator notifications.
...
- Known transient SMTP/connectivity/timeout failures and otherwise-unexpected sender failures receive bounded exponential-backoff retries. Authentication, TLS/configuration, permanent SMTP rejection, or attempt exhaustion dead-letters the delivery. SMTP command rejection text is discarded; structured error logs use only delivery/run/node identity, numeric status, command-error enum, and normalized classification. Status 552 is `smtp.message_too_large`.
...
- Alert/action templates use bounded allow-listed workflow/run/error variables. Email-target templates additionally permit only the single inline `{{data}}` insertion point. Arbitrary headers and arbitrary row expressions remain unsupported. Profiles cap recipients, body/subject size, recipient domains, and concurrent sends. SQL claim admission serializes on the profile row and counts unexpired sending leases, so multiple Worker dispatchers share the same profile admission limit.

Email profiles, rules, outbox persistence, terminal-event integration, the `EmailAlert` action, `EmailTarget` destination, leased dispatcher, MailKit adapter, and a redacted delivery monitor are implemented; build/test/browser and representative SMTP-client validation remain pending for the destination. Production secret-provider integration and a reviewed dead-letter requeue operation remain deployment/operations work.
