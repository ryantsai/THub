...
The migration chain and runtime now implement the immutable workflow/run/step model required by [ADR-0010](adr/0010-durable-leased-workflow-execution.md), the durable Email model required by [ADR-0027](adr/0027-email-workflow-destination.md), and the governed publication model required by [ADR-0011](adr/0011-isolated-governed-data-publications.md).
...
- `AlertDeliveries`: the durable outbox row for a workflow rule, an `EmailAlert` node, or an `EmailTarget` node. It stores the source/run/rule-or-node and originating-step identity, a unique deduplication key, stable message identity, bounded message payload, status, maximum/current attempt counts, next/last attempt timestamps, completion time, lease/heartbeat fields, provider message ID, bounded normalized last error, and row-version concurrency. The message JSON can contain an HTML body or one Base64-encoded CSV attachment for an Email data destination; no new database column is required.

The composite delivery index covers status, next-attempt time, and lease expiry for due-work claims; the deduplication key is unique. Running-run terminal transitions and direct queued-run cancellation commit with their rule deliveries in one THub transaction. The commit rechecks the complete enabled matching rule set and each prepared rule/profile revision. `EmailAlert` and `EmailTarget` nodes use a stable run/node deduplication key and report success once their delivery row is durable; a recovered step attempt therefore observes the existing intent instead of creating a second delivery. `EmailTarget` fails rather than truncates above 10,000 rows, the absolute body limit, or a 10 MiB attachment.

V1 has no `AlertDeliveryAttempts` history table. Each attempt updates the aggregate delivery's attempt count, timestamps, retry/dead-letter state, optional provider identity, and latest safe error. Current structured logs report batch totals, batch exceptions, and payload-free normalized SMTP command rejections rather than an authoritative event for every attempt. The MailKit adapter uses the stable THub MIME Message-ID but currently leaves `ProviderMessageId` empty. The outbox is at-least-once: a crash after SMTP acceptance but before the delivered transition commits can duplicate a message.
...
PD-009 still blocks final retention periods and before/after value classification, so no automatic audit purge is implemented. No table may use the absence of a retention decision as permission to store credentials, bearer-token text, unrestricted row data, or full Email bodies beyond the explicitly bounded governed Email-destination payload accepted by ADR-0027.
