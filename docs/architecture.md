...
Webhook calls and external executables run only through SQL-backed, role-granted trusted actions under [ADR-0020](adr/0020-sql-backed-trusted-workflow-actions.md). Durable Email profiles, workflow-event rules, canvas actions, bounded inline/CSV-attachment data destinations, SQL outbox persistence, leased dispatch, MailKit SMTP delivery, and the management UI implement [ADR-0027](adr/0027-email-workflow-destination.md). The internal, single-host publication and staged-editor slice implements [ADR-0011](adr/0011-isolated-governed-data-publications.md) and [ADR-0021](adr/0021-provider-neutral-governed-publications.md): it includes durable publication metadata, a separate managed-bearer REST host with Swagger/OpenAPI, bounded SQL Server/MySQL/PostgreSQL/Oracle reads, role-granted Spreadsheet editing, and separate Worker apply connections for approved change sets. Immutable versioning and leased workflow execution implement [ADR-0010](adr/0010-durable-leased-workflow-execution.md), with the current node/runtime limits described below.
...
| Connectors | Bounded SQL Server/MySQL/PostgreSQL/Oracle sources and mutation targets plus local and FTP/FTPS CSV/tab-delimited/Excel create-new, append, and replace targets with run-variable file names | Live interoperability matrix, richer mapping, explicitly governed operations, and optional pushdown/spill |
...
| Email delivery | Durable workflow-event, canvas-action, and bounded inline/CSV-attachment data-destination delivery through governed SMTP profiles, with administrator management, normalized SMTP rejection logging, a redacted delivery monitor, and MailKit sender; validation pending for the data destination | Approved production secret-provider integration and a reviewed dead-letter recovery operation |
...
- **Relational target:** SQL Server, MySQL, PostgreSQL, and Oracle targets validate the provider-specific table and column metadata, parameterize every row value, and commit the node in one database transaction. `insert` writes mapped rows. `upsert` requires explicit key metadata that exactly matches the discovered primary key and at least one mapped writable non-key column. `delete` accepts only primary-key mappings and removes only rows whose keys arrive in the input; it never deletes rows merely because they are absent. Null or duplicate mutation keys fail the node before commit. Update-only, replace-table, arbitrary merge predicates, and delete-absent synchronization are not implemented.
...
ADR-0027 preserves terminal workflow-event rules and the `EmailAlert` action and adds the `EmailTarget` destination. Administrators manage enabled delivery profiles and per-workflow rules at `/alerts/email`; profiles constrain the relay, required TLS mode, approved sender, recipient domains, recipient/message/concurrency limits, and an optional credential secret reference. Alert and action templates use only the bounded workflow/run/error variable allow-list. SQL claim admission holds the profile row while counting unexpired sending leases, enforcing `MaximumConcurrentSends` admission across Worker instances.
...
`EmailTarget` consumes exactly one input and fails rather than truncates above 10,000 rows, the existing absolute Email-body bound, or a 10 MiB attachment. Inline mode HTML-encodes schema and cell values into exactly one `{{data}}` placeholder. Attachment mode persists one safely named UTF-8 CSV file. The prepared row payload is part of the durable outbox message and therefore requires the same SQL permission, backup, classification, capacity, and retention treatment as other sensitive message content.

Known transient and otherwise-unexpected sender failures use bounded exponential backoff with deterministic jitter; permanent failures and deliveries that exhaust their attempt limit become dead letters. SMTP command responses are reduced to numeric/enum and normalized classifications before persistence or logging; status 552 becomes `smtp.message_too_large`, and relay response text is never logged. Delivery remains at-least-once: a crash after SMTP acceptance but before the delivered transition commits can produce a duplicate. Attempt count, last-attempt time, current/retry state, an optional provider identity, and the bounded safe last error are kept on `AlertDeliveries`; v1 intentionally has no separate attempt-history table. The MailKit adapter currently supplies the stable THub MIME Message-ID but does not populate `ProviderMessageId`. The default `UnavailableSmtpSecretResolver` rejects every credential reference, so authenticated SMTP fails closed until deployment replaces that registration with an organization-approved `ISecretResolver`.
...
- `/swagger` and `/openapi/v1.json` describe the generic REST authentication, query, paging, and response contract. Publication-specific columns remain protected and are returned only by the bearer-authenticated `/schema` route.
...
- View-granted editor users can export every approved readable source row to a streamed XLSX worksheet. The editor keeps keyset navigation, offers bounded 250/500/1,000/2,000-row pages up to the active version limit, and uses a provider-specific filtered count to show explicit page totals. Editable cells retain the active version's source metadata so Boolean and temporal values receive type-aware controls (including date/time pickers plus parseable text), while nullability, length, precision, and scale are checked before staging.
- edits are normalized, revalidated, and staged in THub; `THub.Worker` claims only approved sets and applies each set in one source transaction with `rowversion` or explicitly selected original-value concurrency. A writable immutable version records a distinct Worker apply connection targeting the same provider/database as its read connection, with a distinct stored credential reference where applicable. Web and REST paths resolve only the read connection.
...
| `Publications`, `PublicationVersions`, `PublicationColumns`, `PublicationColumnForeignKeys` | Stable publication identity and immutable approved relational source/column/FK policy | unique slug/version/alias/ordinal; restricted source-connection/version relationships |
...
- Current publication-host tests cover generic bearer challenges, query-string rejection, duplicate authorization values, process-local admission behavior, and Swagger/OpenAPI contracts. Application/Infrastructure/Web tests cover token services, SQL Server and relational query/mutation planning, connection isolation, role grants, change validation, natural-key immutability, and Spreadsheet mapping. The complete automated suite, EF migration discovery/script generation, Swagger interaction, and desktop/narrow publication-builder rendering were validated on 2026-07-24. Live SQL Server/MySQL/PostgreSQL/Oracle integration must still cover discovery/type/key/foreign-key contracts, cross-publication denial, independent token counts/revocation, fail-closed concurrent metering, schema drift, relational claims/apply, bounds, Problem Details, and rate-limit responses.
